...
def TL_unet_model(input_shape):
    ...
    up1 = Conv2DTranspose(layer_size, (2, 2), strides=(
        2, 2), padding="same")(bridge)
    concat_1 = concatenate(
        [up1, base_VGG.get_layer("block4_conv3").output], axis=3)
    conv6 = Conv2D(layer_size, (3, 3), activation="relu",
    ...
    layer_size //= 2
    up2 = Conv2DTranspose(layer_size, (2, 2), strides=(
        2, 2), padding="same")(conv6)
    concat_2 = concatenate(
        [up2, base_VGG.get_layer("block3_conv3").output], axis=3)
    conv7 = Conv2D(layer_size, (3, 3), activation="relu",
    ...
    layer_size //= 2
    up3 = Conv2DTranspose(layer_size, (2, 2), strides=(
        2, 2), padding="same")(conv7)
    concat_3 = concatenate(
        [up3, base_VGG.get_layer("block2_conv2").output], axis=3)
    conv8 = Conv2D(layer_size, (3, 3), activation="relu",
    ...
    layer_size //= 2
    up4 = Conv2DTranspose(layer_size, (2, 2), strides=(
        2, 2), padding="same")(conv8)
    concat_4 = concatenate(
        [up4, base_VGG.get_layer("block1_conv2").output], axis=3)
    conv9 = Conv2D(layer_size, (3, 3), activation="relu",
...
class ImageMaskSequence(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        images = np.array([self.load_image(path)
...
train_seq = ImageMaskSequence(
    train_dir_images, train_dir_masks, batch_size, target_size)
val_seq = ImageMaskSequence(
    val_dir_images, val_dir_masks, batch_size, target_size)
...
test_seq = ImageMaskSequence(
    test_dir_images, test_dir_masks, batch_size, target_size)
